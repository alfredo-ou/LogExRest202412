...
instance IsTerm a => IsTerm (Relation a) where
   ...
   termDecoder = 
      foldr (<|>) empty
         [ tCon2 t (makeType rt) termDecoder termDecoder
         | (rt, (_, t)) <- relationSymbols
         ] 
...
instance IsTerm a => IsTerm (Equation a) where
   ...
   termDecoder = do
      r <- termDecoder
      case match equationView r of
         Just a  -> return a
         Nothing -> errorStr "not an equation"
...
instance Relational Inequality where
   ...
   flipSides = fromMaybe (error "inequality: flipSides") . match inequalityView
...
instance IsTerm a => IsTerm (Inequality a) where
   ...
   termDecoder = do
      r <- termDecoder
      case match inequalityView r of
         Just a  -> return a
         Nothing -> errorStr "not an inequality"
